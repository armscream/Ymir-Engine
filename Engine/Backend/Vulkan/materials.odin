...
metallic_roughness_build_pipelines :: proc(
	...
	mesh_vert_shader := create_shader_module(
		engine.vk_device,
		#load("/shaders/compiled/mesh_fallback.vert.spv"),
	) or_return
	...
	if new_layout == 0 {
		log.error("material pipeline layout is null after CreatePipelineLayout")
	}
	defer if !ok {
	...
	pipeline_builder_set_vertex_input_for_vertex_struct(&pipeline_builder)
